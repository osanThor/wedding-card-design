...
예비 신랑·신부가 소셜 로그인 후 웹 에디터에서 10분 안에 모바일 청첩장을 만들고 공유할 수 있는 웹앱.

단순 정보 전달형 청첩장을 넘어, 게임식 초대장, 여행권형 초대장, 스토리북, 하객 참여형 롤링페이퍼처럼 사용자와 하객이 상호작용하는 초대 경험 플랫폼으로 확장한다.

- 제작자: 소셜 로그인 후 청첩장 생성·편집·공유
- 하객: 공개 링크로 접속해 예식 정보 확인, 방명록 작성, 계좌 복사, 지도 열기, RSVP, 리액션, 게임, 축하 콘텐츠 남기기
- 핵심 목표: 하나의 예식 정보와 사진, 지도, 계좌, 방명록 데이터를 여러 청첩장 타입에 재사용해 일반 모바일 청첩장, 애니팡 같은 게임형 청첩장, 모바일 여행 티켓형 청첩장 등 다양한 결과물을 제공
...
`profiles` · `invitations` · `invitation_variants` · `invitation_sections` · `invitation_images` · `guestbook_entries` · `accounts` · `rsvps` · `guest_interactions`
...
| 단계 | 주요 기능 |
|---|---|
| MVP | 소셜 로그인, 청첩장 CRUD, 웹 에디터 섹션 편집, 템플릿 5종, 동일 정보 기반 템플릿 전환, 이미지 업로드, 지도, 계좌, 방명록, 공유 링크, 공개 페이지, 이모지 리액션, 게임 1종, 여행권형 템플릿 1종 |
| Beta | RSVP, 음악, QR, 방문 통계, 섹션 순서 변경, 섹션별 상세 스타일 편집, 카카오톡 공유, 하객 축하 카드, 게임 결과 공유 |
...
| 게임식 청첩장 | 하객이 애니팡 같은 모바일 캐주얼 게임 감각으로 미션/퀴즈를 풀며 내용을 확인 | 커플 퀴즈, 밸런스 게임, 카드 뒤집기 |
